...
export default function Cars() {
  ...
  const fetchCars = async () => {
    ...
    try {
      ...
      if (searchTerm.trim()) {
        query = query.or(`id_unico.ilike.%${searchTerm}%,name.ilike.%${searchTerm}%,model.ilike.%${searchTerm}%,license_plate.ilike.%${searchTerm}%,year.eq.${isNaN(Number(searchTerm)) ? 0 : Number(searchTerm)}`);
      }
  ...
  useEffect(() => {
    fetchCars();
  }, [currentPage, entriesPerPage, sortField, sortDirection]);
